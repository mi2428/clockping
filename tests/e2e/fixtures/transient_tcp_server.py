#!/usr/bin/env python3
"""TCP target that intentionally stops listening while its container stays up."""
...
def serve_until_down(host: str, port: int, accepts_before_down: int, stop: threading.Event) -> None:
    """Accept a fixed number of TCP connections, then close the listener."""
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as listener:
        ...
        print(
            f"transient tcp server listening on {host}:{port}; "
    ...
    print(f"transient tcp server stopped listening on {host}:{port}", flush=True)
